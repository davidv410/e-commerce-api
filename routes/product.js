...
router.post('/', protect, upload.array('images', 10), async (req, res) => {
    try{
        ...
        if(req.files || req.files.length > 0){
            await uploadProductImage(req.files, insertedProduct.id)
        }
...
router.patch('/:id', protect, async (req, res) => {
    try{
        ...
        const [updateProduct] = await db.update(product).set(result.data).where(and(eq(product.id, productId), eq(product.userId, req.user.id))).returning()
...
router.delete('/:id', protect, async (req, res) => {
    try{
        ...
        const [removeProduct] = await db.delete(product).where(and(eq(product.id, productId), eq(product.userId, req.user.id))).returning()
